...
async function companyPayload(companyId: string, year = 2026, month = 6) {
  const result = await pool.query(
...
export async function handleManagement(req: ApiRequest, res: ApiResponse): Promise<boolean> {
    ...
    if (!body) return true;
     const ownEmployee = await pool.query(
       `SELECT e.id FROM employees e
        JOIN company_assignments ca ON ca.company_id = e.company_id
        WHERE ca.user_id=$1 AND e.deleted_at IS NULL
  ...
  if (params) {
    ...
    const values = parseMonthYear(req, res);
    if (!values) return true;
    const company = await companyPayload(req.params.companyId, values.year, values.month);
    if (!company) {
      sendError(res, 404, "NOT_FOUND", "Company not found.");
      return true;
    }
  ...
  if (params) {
    ...
    const account = await pool.query(
      `SELECT u.id
       ...
       WHERE ca.company_id=$1 AND u.role='SECURITY_GUARD'
  ...
  if (params) {
    ...
    try {
      ...
      // Keep a guard's login identity aligned with the employee record.
      await client.query(
  ...
  if (params) {
    ...
    const rows = result.rows.map((row) => ({ companyId: row.company_id, companyName: row.company_name, ...financialPayload(row), balance: money(row.balance), profit: money(row.profit) }));
  ...
  if (params) {
    ...
    await pool.query(
      ...
      [`account-${value.year}-${value.month}-${value.companyId}`, value.companyId, value.month, value.year,
        value.totalBilling, value.totalReceiving, value.cashReceived, value.salary, value.balance,
        value.expense, value.dressStock, value.profit],
    );
